...
runLiftMask t predicate = do
  ...
  let liftMask = fetchBuiltin (selectorType ->> TList TUnit ->> TList TBool) LiftMask
...
fetchBuiltin rett Return = VBuiltin $ \v -> do
  ...
  case t of
    (TDice _) -> return $ VDice $ return v
    (TList _) -> return $ VList $ one v
    (TPool _) -> return $ VPool (return . return $ v) (return v)
...
fetchBuiltin t LiftMask =
  let getLiftMaskType :: WeedType -> Either EvaluationError Bool
      getLiftMaskType ty = case N.toList $ unwrapFunction ty of
        [TFunction (TList _) (TList TBool), TList _, TList TBool] -> return True
        [TFunction _ TBool, TList _, TList TBool] -> return False
...
fetchBuiltin _ Poolify = liftValue2 poolify
  where
    ...
    poolify (VNumber _) e = throwError $ TypeError (TPool TNumber) e
